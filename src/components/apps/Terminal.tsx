  ...
  help = () => {
    const help = (
      <ul className="list-disc ml-6 pb-1.5">
        <li>
          <span text-red-400>ai [question]</span> - Ask the AI assistant a question
        </li>
        <li>
  ...
  aiChat = (args?: string) => {
    if (!args) {
      this.generateResultRow(
        this.curInputTimes,
        <div>
          <div>Welcome to AI Assistant! Type 'ai help' to see available commands.</div>
          <div>Ask me anything and I'll try to help!</div>
        </div>
      );
      return;
    }
    ...
    if (args.toLowerCase() === 'help') {
      const helpText = (
        <div>
          <div>AI Assistant Commands:</div>
          <ul className="list-disc ml-6">
            <li><span className="text-blue-300">ai help</span> - Show this help message</li>
            <li><span className="text-blue-300">ai hello</span> - Get a greeting</li>
            <li><span className="text-blue-300">ai time</span> - Get current time</li>
            <li><span className="text-blue-300">ai [your question]</span> - Ask me anything</li>
          </ul>
        </div>
      );
      this.generateResultRow(this.curInputTimes, helpText);
      return;
    }

    // Simple response logic - you can expand this with more complex AI integration
    let response = "I'm not sure how to respond to that. Type 'ai help' for available commands.";

    const lowerArgs = args.toLowerCase();
    if (lowerArgs.includes('hello') || lowerArgs.includes('hi')) {
      response = "Hello! How can I assist you today?";
    } else if (lowerArgs.includes('time')) {
      response = `The current time is: ${new Date().toLocaleTimeString()}`;
    } else if (lowerArgs.includes('thank')) {
      response = "You're welcome! Is there anything else I can help with?";
    } else if (lowerArgs.includes('help')) {
      response = "I can help you with various tasks. Try asking me about time, or just say hello!";
    }

    // Add typing effect for the response
    this.generateResultRow(
      this.curInputTimes,
      <div className="text-blue-300">AI: {response}</div>
    );
  };
